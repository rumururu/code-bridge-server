...
2. **RevenueCat REST**: if ``REVENUECAT_SECRET_API_KEY`` is set and the
   ...
   ``code_bridge_pro`` entitlement. Successful lookups are cached for 5
   minutes per app user id.
3. **Stale cache fallback**: if RevenueCat is temporarily unavailable
   (5xx, network errors), entries up to 1 hour old are served with a
   ``logger.warning`` so the user is not booted out during an upstream
   blip.
...
class EntitlementService:
    ...
    def is_entitled(self, app_user_id: Optional[str]) -> EntitlementResult:
        ...
        if normalized_user_id is None:
            if not api_key:
                # No key, no user — pure anonymous, not configured.
        ...
        # 4) RevenueCat not configured AND no override.
        return EntitlementResult(
            active=False,
