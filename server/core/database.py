...
# Common IDE project directory names (relative to home directory)
IDE_PROJECT_DIRECTORIES = [
    # JetBrains IDEs
    "AndroidStudioProjects",  # Android Studio
    "PycharmProjects",        # PyCharm
    "IdeaProjects",           # IntelliJ IDEA
    "WebstormProjects",       # WebStorm
    "CLionProjects",          # CLion
    "GolandProjects",         # GoLand / GoLand
    "PhpstormProjects",       # PhpStorm
    "RubyMineProjects",       # RubyMine
    "DataGripProjects",       # DataGrip
    "RiderProjects",          # Rider
    # Eclipse
    "eclipse-workspace",
    # NetBeans
    ...
    # Xcode
    "Developer",
    # VS Code workspaces (common convention)
    "VSCodeProjects",
    "vscode-projects",
    "Projects",
    "projects",
...
def get_ide_project_directories() -> list[str]:
    """Find existing IDE project directories in the user's home folder.

    Returns:
        List of absolute paths to existing IDE project directories.
    """
    home = Path.home()
    found_dirs = []

    for dir_name in IDE_PROJECT_DIRECTORIES:
        dir_path = home / dir_name
        if dir_path.exists() and dir_path.is_dir():
            found_dirs.append(str(dir_path.resolve()))
...
def migrate_accessible_folders_from_projects() -> int:
    ...
    Priority order:
    1. If projects exist, use their parent directories
    2. If no projects, check for common IDE project directories
    3. If no IDE directories found, use home directory as fallback
    ...
    # If projects exist, extract their parent directories
    if projects:
    ...
    # No projects - check for IDE project directories
    ide_dirs = get_ide_project_directories()
    if ide_dirs:
        count = 0
        for folder in ide_dirs:
            if folder_db.add(folder):
                logger.info("Added IDE project directory as accessible folder: %s", folder)
                count += 1

        if count > 0:
            logger.info("Added %d IDE project directories as default accessible folders", count)
        return count

    # Fallback: add home directory
